...
def _has_index(schema: str, table: str, index: str) -> bool:
    indexes = sa.inspect(op.get_bind()).get_indexes(table, schema=schema)
    return any(row["name"] == index for row in indexes)
...
def upgrade() -> None:
    ...
    if not _has_table("control", "ingestion_runs"):
    ...
    if not _has_index("control", "ingestion_runs", "ix_ingestion_runs_source_started"):
        op.create_index("ix_ingestion_runs_source_started", "ingestion_runs", ["source", "started_at"], schema="control")
    if not _has_table("raw", "source_records"):
    ...
    if not _has_index("raw", "source_records", "ix_raw_source_period_geo"):
        op.create_index("ix_raw_source_period_geo", "source_records", ["source", "period", "geography_code"], schema="raw")
    if not _has_table("analytics", "geographies"):
    ...
    if not _has_table("analytics", "indicator_observations"):
    ...
    if not _has_index("analytics", "indicator_observations", "ix_indicator_lookup"):
